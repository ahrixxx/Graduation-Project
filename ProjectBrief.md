...
|---|
| **데이터 수집 및 정제** |
| - 외부 금융/경제 지표 API (금리, 환율, 유가, 주요 섹터 지수)와 주식/ETF 시세 데이터 수집 |
| - 결측치 처리 (Forward/Backward Fill, Interpolation) 및 이상치 제거 |
| - 정규화/스케일링을 통해 시계열 비교 가능하도록 전처리 |
| **포트폴리오 입력 및 매핑** |
| - 사용자 종목 입력 또는 CSV 업로드 기능 |
| - 종목 → 산업/섹터 자동 매핑 (DB/증권사 분류 체계 활용) |
| - 포트폴리오 비중 계산 및 데이터베이스 저장 |
| **리스크 이벤트 탐지** |
| - 이동평균선 이탈, 단기 변동성 급등, 최대 낙폭(Max Drawdown) 등 지표 계산 |
| - 임계치 기반 규칙 엔진으로 “Low / Medium / High” 리스크 구간 산출 |
| - 포트폴리오 종목별, 섹터별 신호 통합 |
| **뉴스·리스크 요약 (AI 요소)** |
| - OpenAI GPT API 활용: 뉴스 기사 → 간단 요약문 생성 |
| - “이 뉴스가 내 포트폴리오/섹터에 어떤 의미가 있는가?” → 설명 자동화 |
| - 투자 용어(이평선, PER, 변동성 등) 등장 시 초보자 친화적 해설 자동 생성 |
| **데이터 융합 및 인사이트 생성** |
| - 리스크 이벤트 신호 + 뉴스/지표 해석 결과를 통합 |
| - GPT로 “오늘의 Top 3 리스크 요인” 카드 및 개인화 코멘트 생성 |
| **시각화 및 알림** |
| - Plotly 기반 대화형 시각화 (라인 차트, 히트맵, 게이지 차트) |
| - React 대시보드 UI에 카드 형태로 리스크 신호/인사이트 표시 |
| - 알림 스케줄러(FastAPI + Celery)로 이벤트 발생 시 사용자에게 즉시 전달 |
